...
def incentive_allocation_old(num,gamma,x,test_mode=False):
    """
    激励分配迭代算法
    外部参数：
        num: 参与矿工数量
...
def incentive_allocation(num,gamma,p,pm,test_mode=False):
    """
    激励分配迭代算法
    外部参数：
        num: 参与矿工数量
    ...
    while DIF>Thres or round<100:
        #对集合I中的每个i
        for i in range(num):
            q=np.sum(pm[:i])+np.sum(pm[i+1:])
            M[i]=np.sqrt(gamma/q)*Dp/T
            pm[i]=np.sqrt(M[i]*q/gamma)-q
            if(pm[i]<=0):
                pm[i]=0
            elif(pm[i]>p[i]):
                pm[i]=p[i]
        ...
        if test_mode==True:
            if round%20==0:
                print("This is round:", round,", with a DIF of",DIF)
                print("M:", DIF+np.mean(M)**2)
                print("sum_x:", sum(pm),'\n',pm,'\n')
    return pm,DIF+np.mean(M)**2